<template>
  <div>
    <div class="mt-3 mb-1 links">
      <add-topic class="px-1" :forumId="forumId" ></add-topic>
      <edit-forum v-if="forum.creator === email" class="px-1" :form="forum">
        <template>Edit Forum '{{ forum.name }}'</template>                
      </edit-forum>
      <b-link v-if="forum.creator === email" @click="deleteConfirm()" class="px-1">Delete forum</b-link>
    </div>    
    <b-container fluid>    
      <b-row class="header">
...
export default {
  ...
  methods: {
    ...
    params(topicId) {
      return { name: 'posts', params: { forumId: this.forumId, topicId } }
    },
    deleteConfirm() {
  ...
  beforeRouteEnter (to, from, next) {       
    Promise.all([
      store.dispatch(`topic/${ FETCHTOPICS }`, to.params.forumId),
      store.dispatch(`topic/${ FETCHTOPICSFORUM }`, to.params.forumId)
    ]).then(() => next())
  }, 
  components: {
    addTopic: AddTopic,
