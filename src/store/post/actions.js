...
import { FETCHTOPIC, ADDTOPICPOSTID, UPDATETOPIC, DELETETOPICPOSTID } from '../topic/action-types'
import { FETCHPOSTS, FETCHPOSTSTOPIC, ADDPOST, UPDATEPOST, 
         REPLYPOST, DELETEPOST, DELETEPOSTBYTOPIC, ISANSWER } from './action-types'
import { STOREPOSTS, STORETOPIC, UPDATE, UPDATEANSWERED, REPLY } from './mutation-types'
import { FAILED } from '../../store'

let v = new Vue()

export default {
  [FETCHPOSTS] ({ commit, dispatch }, topicId) {           
    dispatch(`topic/${ FETCHTOPIC }`, topicId, { root: true  })
      .then(({ data }) => {
         dispatch(`topic/${ UPDATETOPIC }`, { id: topicId, topic: { views: data.views + 1 } }, { root: true  })
      })
      .catch(e => dispatch(FAILED, e, { root: true }))
      
    return v.$forums.fetchPosts(topicId)
      .then(({ data }) => {       
        const posts = []
  ...
  [FETCHPOSTSTOPIC] ({ commit, dispatch }, topicId) {
    return dispatch(`topic/${ FETCHTOPIC }`, topicId, { root: true })
      .then(({ data }) => {
        data.id = topicId
   ...
   [ADDPOST] ({ dispatch }, { post, topicAdded }) {
      ...
      return v.$forums.addPost({
        ...
        .then(postId => dispatch(`topic/${ ADDTOPICPOSTID }`, { topicId, postId }, { root: true }))            
        .then(() => v.$forums.fetchForum(forumId))
        .then(({ data }) => data.posts)
        .then(posts => dispatch(`forum/${ UPDATEFORUM }`, { 
          id: forumId,
          lastPost: title, 
    ...
    [ISANSWER] ({ commit, dispatch }, post) {
      ...
      return v.$forums.updatePost(post.id, { answered })
    ...
    [UPDATEPOST] ({ commit, dispatch }, post)  {
      ...
      return v.$forums.updatePost(id, { title, content })        
     ...
     [REPLYPOST] ({ commit, dispatch }, post) {
      ...
      return dispatch(ADDPOST, { post, topicAdded: false })
        .then(() => dispatch(`topic/${ FETCHTOPIC }`, post.topicId, { root: true  })) 
        .then(({ data }) => dispatch(`topic/${ UPDATETOPIC }`, { id: post.topicId, topic: { replies: data.replies + 1 } }, { root: true  }))
