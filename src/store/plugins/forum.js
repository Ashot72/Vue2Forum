...
export default {
  install (Vue, { storageKey, key, loader }) {
    ...
    Vue.axios.forums.interceptors.response.use(
      res => {
        loader(false)
        return res
      },
  ...
  updateForumAndTopicId (topics, id, topicsId) {
    return Vue.axios.forums.patch(`/forums/${id}.json`, {
      topics,
      topicsId
    })
  },
  ...
  fetchTopics (forumId) {
    return Vue.axios.forums.get(
      `/topics.json?orderBy="forumId"&equalTo="${forumId}"`
    )
  },
  fetchPost (id) {
  ...
  updateTopicAndPostId (replies, id, postsId) {
    return Vue.axios.forums.patch(`/topics/${id}.json`, {
      replies,
      postsId
    })
  },
  ...
  fetchPosts (topicId) {
    return Vue.axios.forums.get(
      `/posts.json?orderBy="topicId"&equalTo="${topicId}"`
    )
  },
  addPost (post) {
